...
| `/skills` | 登録された skill 一覧 |
| `/plugins` | インストール済プラグイン一覧 |
| `/mcp` | アクティブな MCP ツール一覧 |
| `/model [id]` | アクティブモデルを切替（引数なしでピッカー、tools 対応モデルのみ選択可） |
| `/models` | API から取得可能な Mercury モデル一覧 |
...
リリース済 (v0.1–v0.3) と今後の見通し (v1.0 へ向けたストリーミング、サブエージェント並列、プラグインマーケット、マルチモデルゲートウェイ等) は [ROADMAP.md](./ROADMAP.md) を参照。
